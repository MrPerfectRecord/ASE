...
export default function Footer() {
  ...
  // Render a row — while loading, show an invisible placeholder so layout
  // stays stable without flashing stale hardcoded values. After load, skip
  // rows whose value is empty.
  const loading = contact === null;
  const row = (icon: JSX.Element, value: string) => {
    if (!loading && !value) return null;
    return (
      <li className="flex items-center gap-3">
        <span className="w-4 h-4 text-white/50 flex-shrink-0">{icon}</span>
        <span
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-12">
          ...
          <div>
            ...
            <ul className="space-y-4">
              {row(phoneIcon, contact?.phone ?? "")}
              {row(emailIcon, contact?.email ?? "")}
              {row(pinIcon, contact?.address ?? "")}
            </ul>
          </div>
        </div>
